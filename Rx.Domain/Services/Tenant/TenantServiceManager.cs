...
namespace Rx.Domain.Services.Tenant
{
    public sealed class TenantServiceManager : ITenantServiceManager
    {
        ...
        public TenantServiceManager(ITenantDbContext tenantDbContext,ILogger<TenantServiceManager> logger,IPaymentService paymentService ,IMapper mapper,IBackgroundJobClient backgroundJobClient,IRecurringJobManager recurringJobManager,IBlobStorage blobStorage,IHttpClientFactory httpClientFactory)
        {
            ...
            _subscriptionService = new Lazy<ISubscriptionService>(() => new SubscriptionService(tenantDbContext, logger, mapper,backgroundJobClient,paymentService,recurringJobManager,httpClientFactory));
            ...
            _addOnUsageService = new Lazy<IAddOnUsageService>(() => new AddOnUsageService(tenantDbContext, logger,mapper,paymentService,httpClientFactory));
        }
