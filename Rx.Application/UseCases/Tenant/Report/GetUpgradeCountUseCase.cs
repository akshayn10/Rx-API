...
public record GetUpgradeCountUseCase():IRequest<IEnumerable<Stats>>;
public class GetUpgradeCountUseCaseHandler : IRequestHandler<GetUpgradeCountUseCase, IEnumerable<Stats>>
{
    private readonly ITenantDbContext _tenantDbContext;

    public GetUpgradeCountUseCaseHandler(ITenantDbContext tenantDbContext)
    {
        _tenantDbContext = tenantDbContext;
    }
    public async Task<IEnumerable<Stats>> Handle(GetUpgradeCountUseCase request, CancellationToken cancellationToken)
    {
        var monthsRequired = 6;
        var result =await _tenantDbContext.SubscriptionStats!
            .Where(s=>s.Change=="upgrade")
